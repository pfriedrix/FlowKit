...
final class BindingTests: XCTestCase {
    ...
    @MainActor
    func testCustomGetterBinding_returnsConstantFromGetter() {
        let store = createStore()
        ...
        XCTAssertEqual(binding.wrappedValue, "Custom Value")
    }

    @MainActor
    func testCustomGetterBinding_settingValueDispatchesAction() async throws {
        let store = createStore()
        ...
        try await waitForStateChange(timeout: 1.0) {
            store.state.someStateProperty == "Updated Value"
        }
        XCTAssertEqual(store.state.someStateProperty, "Updated Value")
    }

    @MainActor
    func testCustomGetterBinding_getterIgnoresStateChanges() async throws {
        let store = createStore()
